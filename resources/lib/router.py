...
routes: dict = {
    ...
    "/menu/{mode}/{genre}/{season_filter}": PARAMETER_ROUTE_MODE,
    "/menu/{mode}/{genre}/{season_filter}/offset/{offset}": PARAMETER_ROUTE_MODE,
...
def get_matching_routes_from_mode(searching_mode: str) -> list:
    # TODO: Cache it
    routes_by_mode: dict = None
    if routes_by_mode is None:
        result = {}
        for pattern, mode in routes.items():
            if not result.get(mode):
                result[mode] = []
            result.get(mode).append(pattern)
        routes_by_mode = result
...
def get_params_from_route(route: str) -> list:
    # TODO: Cache it
    params_by_route: dict = None
    if params_by_route is None:
        result = {}
        for pattern, mode in routes.items():
            result[pattern] = get_params_from_pattern(pattern)
        params_by_route = result
